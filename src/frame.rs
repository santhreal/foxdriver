...
async fn frame_viewport_offset(page: &Page, target: &crate::FrameId) -> Result<(f64, f64)> {
    ...
    for i in 0..chain.len().saturating_sub(1) {
        ...
        let off = kids
            ...
            .map(|(_, _, _, x, y)| (*x, *y))
            .or_else(|| {
                // Index unavailable (cross-origin quirk): fall back to src/id.
                let m = lookup_iframe_offset(&kids, &child.url, -1);
                (m != (0.0, 0.0)).then_some(m)
            });
...
mod tests {
    ...
    fn lookup_iframe_offset_matches_by_id() {
        ...
        assert_eq!(lookup_iframe_offset(&offsets, "iframe-0", -1), (10.0, 20.0));
    }
    ...
    fn lookup_iframe_offset_index_mismatch_falls_back_to_first_match() {
        ...
        // Requesting index 99 of "a.html" doesn't exist, but with
        // iframe_idx=-1 it falls back to first src match.
        assert_eq!(lookup_iframe_offset(&offsets, "a.html", 99), (0.0, 0.0));
        assert_eq!(lookup_iframe_offset(&offsets, "a.html", -1), (10.0, 20.0));
    }
    ...
    fn lookup_iframe_offset_negative_beyond_minus_one_treated_as_fallback() {
        ...
        assert_eq!(lookup_iframe_offset(&offsets, "x", -5), (5.0, 6.0));
    }
