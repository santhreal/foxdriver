...
impl Drop for Page {
    fn drop(&mut self) {
        ...
        // terminate it explicitly so a self-spawned reynard/Camoufox never leaks.
...
pub fn proxy_prefs(proxy: &ProxyConfig) -> String {
    ...
    // silently blows the operator's cover even when every HTTP byte is proxied.
...
pub async fn launch_firefox_self_managed(config: FoxBrowserConfig) -> Result<Page> {
    ...
    command.args(&args).env("MOZ_LAUNCHER_PROCESS", "0");
